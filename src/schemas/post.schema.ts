...
export const createPostSchema = z.object({
  body: z.object({
    ...
    title: z.string().min(3, 'El título debe tener al menos 3 caracteres').max(100, 'Máximo 100 caracteres').trim(),
    description: z.string().min(10, 'La descripción debe tener al menos 10 caracteres').max(2000, 'Máximo 2000 caracteres').trim(),
    city: z.string().min(2, 'Debes indicar una ciudad'),
    price: z.number().positive('El precio debe ser positivo').optional(),
    currency: z.string().optional(),
    images: z.array(z.string().url('URL de imagen inválida')).max(4, 'Máximo 4 imágenes').optional(),
...
export const reportPostSchema = z.object({
  body: z.object({
    reason: z.string().min(5, 'Debes indicar el motivo del reporte').max(200, 'Máximo 200 caracteres'),
